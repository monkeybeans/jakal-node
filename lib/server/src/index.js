...
.use('/', announceOrLeave)
.use((req, res, next) => {
  const isApiRequest = req.url.match(API_PATH) !== null;
  if (isApiRequest) {
    next();
  } else {
    const username = req.publicUserData && req.publicUserData.username
    const isAuthenticateRequest = req.url.match(AUTH_PATH) !== null;

    if (isAuthenticateRequest && username) {
      res.redirect(302, `/`);
    } else if (isAuthenticateRequest) {
      loadApp(req, res);
    } else if (username) {
      loadApp(req, res);
    } else {
