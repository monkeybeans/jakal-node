...
const CONTEXT_PATH = '/jakal-web-BETA';
const API_PATH = `${CONTEXT_PATH}/api/v1`;
...
function accessLog(req, res, next) {
  log(`${req.method} ${req.originalUrl} from ${req.publicUserData && req.publicUserData.username}`);
  next();
}
...
.use(accessLog)
.get('/ping', ping)
.use(`${CONTEXT_PATH}/assets`, express.static(STATIC_PATH))
.use(CONTEXT_PATH, announceOrLeave)
.use((req, res, next) => {
