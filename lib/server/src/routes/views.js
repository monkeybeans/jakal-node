...
<html lang="en">
  ...
  <body>
    ...
    <script src="/jakal-web-BETA/assets/browser.bundle.js?${RANDOM}"></script>
  </body>
</html>`);
...
async function loadApp(req, res) {
  ...
  let endorsed = await getFreshSuggestions({ settings, today: new Date() });
  let suggestions = await getEndorsedSuggestions();
  ...
  const RootElment = React.createElement(LoadRouter(preloadedState), { location, context });
