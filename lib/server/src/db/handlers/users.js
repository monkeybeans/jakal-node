...
export function getUsersNotVoted({ votingStartDay, now = new Date() }) {
  const votingStartDate = rewindDateToDay(now, votingStartDay);
  const midnightDate = new Date(votingStartDate.year(), votingStartDate.month(), votingStartDate.date());

  return UserModel
    .find(
      { lastVoting: { $lt: midnightDate } },
      { emails: 1, username: 1, _id: 0 },
      { lean: true }
    );
}
...
export function touchSession({ username, session }) {
  return UserModel
  .findOneAndUpdate({ username}, { session }, { new: true});
}
...
export function checkIfVotingAllowed(session) {
  ...
  return UserModel
  .findOne({ session, lastVoting: { $lt: midnightDate } })
  .then(eligible => {
    if (eligible === null) {
      return false;
    }
