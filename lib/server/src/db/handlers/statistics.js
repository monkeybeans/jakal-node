...
const getOldestSuggestion = () => {
  return SuggestionModel.find().sort({ 'submitter.time': 1}).limit(1).then(result => result[0] || {});
}

const getStatistics = async () => {
  const lastYear = moment().subtract(1, 'year');

  const [mostActiveSubmitters, votingNumbers, oldest] = await Promise
    .all([getMostActiveSubmitters(3), getAverageVotesSinceDate(lastYear), getOldestSuggestion()]);
    ...
    return {
      stats: [
        ...
        {
          label: 'Time since first suggestion',
          data: {
            meta: 'In days',
            value: moment().diff(oldest.submitter.time, 'days'),
          },
        },
        {
          label: 'votes / month',
