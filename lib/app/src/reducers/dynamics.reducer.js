...
const fetchSuggestions = () => (dispatch, getState, { api }) => {
  ...
  api
    .get('/jakal-web-BETA/api/v1/dynamics/suggestions')
    .then(({ data }) => {
      dispatch({
...
const sendSuggestion = ({ name, description }) => (dispatch, getState, { api }) => {
  ...
  api
    .post('/jakal-web-BETA/api/v1/dynamics/suggestion', { name, description })
    .then(() => {
...
const sendSuggestionVote = id => (dispatch, getState, { api }) => {
  ...
  api
    .post(`/jakal-web-BETA/api/v1/dynamics/suggestion/${id}/vote`)
    .then(() => {
